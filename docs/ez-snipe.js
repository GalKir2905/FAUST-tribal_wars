                ...
                libs.forEach((l, i) => {
                    if (l.indicator && l.indicator()) {
                        ...
                        if (++numPending == libs.length) {
                            libs.forEach((l, si) => l.onLoad && (!l.indicator || !l.indicator()) ? l.onLoad(requestData[si]) : null);
                            onDone();
                        }
                        return;
                    }
        ...
        */
        {
            ...
            function extractUserTimeInput(timeString) {
                ...
                } else if (match = translate(INTL.T_TODAY_AT, { type: 'regex' }).exec(timeString)) {
                ...
                } else if (match = translate(INTL.T_TOMORROW_AT, { type: 'regex' }).exec(timeString)) {
                ...
                } else if (match = translate(INTL.T_ON_AT, { type: 'regex' }).exec(timeString)) {
            ...
            function parseTextInput(textInput) {
                ...
                textInput.split().forEach((l, i) => {
                    ...
                    var match,
                        villageName,
                        landingDate, landingTime, landsAt;
                    ...
                    if (match = l.match(/Village\:([\w\s])+\(\.+\ K\d+\)/i)) {
                        ...
                        landingDate = null;
                        landingTime = null;
                    } else if (match = l.match(/(\w+\s+\d+,?\s+\d+)\s+(\d+:\d+:\d+(?:\d+))/i)) {
                        landingDate = match[1];
            ...
            function parseOverviewVillaRows($rows) {
                ...
                $rows.each((i, el) => {
                    ...
                    $availInVillaRow.each((i, el) => availTroopsInVilla.push(parseInt($(el).text())));
    
                    var $troopsInVillaRow = $el.find('tr:nth-child(2) td.unit-item');
                    var troopCountsInVilla = [];
                    $troopsInVillaRow.each((i, el) => troopCountsInVilla.push(parseInt($(el).text())));
                    ...
                    villas.push({
                        ...
                        link: villaUrl,
                        name: villaName,
                        rallyPointLink: makeSitterUrl(`${window.location.host}/game.php?village=${currentId}&screen=place`),
        ...
        */
        {
            function getVillageDataById(villageId, callback) {
                ...
                $.get(url, (data) => {
                    ...
                    var coord = $doc.find('table.vis:nth-child(1) tr:nth-of-type(3) td:nth-of-type(2):first').text().trim();
                    console.log(name);
                    console.log(coord);
                    if (!name || !coord) {
                        alert('Village with ID ' + villa.id + ' does not exist! (Or the script is broken..)');
                        return;
                    }
            ...
            function findVillaByCoord(villas, coord) {
                var result = null;
                villas.forEach((v) => v.coord.x == coord.x && v.coord.y == coord.y ? result = v : null);
                return result;
            }
        }
        ...
        */ 
        {
            function toggleArchers(useArchers) {
                ...
                if (!useArchers && archerUnit) {
                    console.log('World has archers disabled, removing archers from parser');
        ...
        */
        {
            function calculateWalkTime(unitType, sourceCoord, destCoord) {
                var dist = Math.sqrt(Math.pow(sourceCoord.x - destCoord.x, 2) + Math.pow(sourceCoord.y - destCoord.y, 2));
                return dist * unitType.travelSpeed / TW.gameSpeed / TW.unitSpeed;
            }

            function calculateTroopStatsByUnitSpeed(unitCounts, baseUnitType, build_) {
                var paladinIndex = 10;
                ...
                unitCounts.forEach((cnt, i) => {
                    ...
                    if (unit.travelSpeed > baseUnitType.travelSpeed && baseUnitType.name != 'Paladin') {
                        return;
                    }
                ...
                result.totalDef = result.totalInfDef + result.totalCavDef + result.totalArDef;
                return result;
            }
                ...
                sourceVillas.forEach((vil, vi) => {
                    incomings.forEach((inc, ii) => {
                        ...
                        vil.ownTroopsInVilla.forEach((cnt, ti) => {
                            ...
                            var travelTimeMinutes = calculateWalkTime(unitType, vil.coord, inc.destination.coord);
                            ...
                            launchTime.setUTCDate(launchTime.getUTCDate() - travelTimeParts.days);
                            launchTime.setUTCHours(launchTime.getUTCHours() - travelTimeParts.hours);
                            launchTime.setUTCMinutes(launchTime.getUTCMinutes() - travelTimeParts.minutes);
                            launchTime.setUTCSeconds(launchTime.getUTCSeconds() - travelTimeParts.seconds);

                            var hoursToMs = 60 * 60 * 1000;
                            var timeUntilLaunch = launchTime.valueOf() - currentTime.valueOf();
                            if (timeUntilLaunch < 0 || timeUntilLaunch > ezsSettings.maximumTimeUntilLaunch * hoursToMs) {
                                return;
                            }
            ...
            function makeBasicSnipeBuild(snipeOptions) {
                ...
                TW.unitTypes.forEach((type, i) => {
                    if (snipeOptions.unitType.name != 'Paladin' && type.name == 'Paladin') {
                        //  Don't include paladin if it wasn't specifically chosen for the snipe
                        return;
                    } else if (snipeOptions.unitType.name == 'Paladin' && type.build == 'Offensive' && !ezsSettings.offensiveSnipes) {
                        // Don't include offensive units if we're sending paladin
                        return;
                    }
                    //  Ignore speed limits if paladin is being sent with snipe
                    if (snipeOptions.sourceVillageData.ownTroopsInVilla[i] > 0 && (type.travelSpeed <= snipeOptions.unitType.travelSpeed || snipeOptions.unitType.name == 'Paladin')) {
                        totalAllowedPop += snipeOptions.sourceVillageData.ownTroopsInVilla[i] * type.population;
                        allowedUnits.push(i);
                    }
                });
                ...
                snipeOptions.sourceVillageData.ownTroopsInVilla.forEach((cnt, i) => {
                    ...
                    if (TW.unitTypes[i].build == "Offensive" && !ezsSettings.offensiveSnipes) {
        ...
        */
        {
            function getManualIncomings() {
                var manualIncomings = JSON.parse(localStorage.getItem(LS_KEY_MANUAL_INCOMINGS)) || [];
                manualIncomings.forEach((inc) => inc.arrival = new Date(Date.parse(inc.arrival)));
            ...
            function saveManualIncoming(inc) {
                ...
                localStorage.setItem(LS_KEY_MANUAL_INCOMINGS, JSON.stringify(manualIncomings));
            }
            ...
            function deleteManualIncoming(inc) {
                ...
                if (bestMatchIdx < 0) {
                    console.warn("Couldn't find an incoming in localStorage matching the deleted incoming");
                    return;
                }
        ...
        */
        {
            ...
            function makeBbCodeOutput($container, displayType, currentTime, snipes) {
                ...
                orderedSnipes.sort((a, b) => a.schedule.launchTime.valueOf() - b.schedule.launchTime.valueOf());
    
                var formatNumber = (num) => num.toString().length == 1 ? "0" + num.toString() : num.toString();
    
                function formatTravelTime(time) {
                    return `${formatNumber(time.days)}:${formatNumber(time.hours)}:${formatNumber(time.minutes)}:${formatNumber(time.seconds)}`;
                }
                ...
                endTime.setUTCHours(endTime.getUTCHours() + ezsSettings.maximumTimeUntilLaunch);
                var timeRange = translate(INTL.T_SNIPE_TIME_RANGE, { currentTime: formatDateTime(currentTime), endTime: formatDateTime(endTime) });
    
                var $textarea = $(`<textarea style="${textareaStyle}">`);
                $container.append($textarea);
    
                switch (displayType) {
                    case 'source': makeSourceOutput($textarea); break;
                    case 'destination': makeDestinationOutput($textarea); break;
                    case 'all': makeAllOutput($textarea); break;
                }
                ...
                function makeSourceOutput($target) {
                    ...
                    orderedSnipes.forEach((s) => {
                        ...
                        sourceVillageSnipes.forEach((svs) => {
                            if (snipeData == null && svs.source == s.schedule.sourceVillageData) {
                                snipeData = svs;
                            }
                        });
                    ...
                    sourceVillageSnipes.forEach((vs) => {
                        ...
                        var headerFormat;
                        if (isCompact) {
                            headerFormat = [translate(INTL.T_TARGET), translate(INTL.T_LAUNCH_TIME), translate(INTL.T_LANDS_AT), translate(INTL.T_TROOP_REQ)];
                        } else {
                            headerFormat = ['#', translate(INTL.T_TARGET), translate(INTL.T_LAUNCH_TIME), translate(INTL.T_LANDS_AT), '', translate(INTL.T_TROOPS)];
                        }
    
                        var headerText = `${translate(INTL.T_SNIPES_FROM_SRC, { source: source.coordText })} (${snipes.length})`;
                        if (isCompact) {
                            lines.push(headerText);
                        } else {
                            lines.push(`[b]${headerText}[/b]`);
                        }
                        lines.push(makeBbCodeTable(headerFormat, snipes.map((s, i) => {
                            var row = [];
                            if (!isCompact) {
                                row.push(i + 1 + '.');
                            }
                            ...
                            row.push(`[url=${s.schedule.sourceVillageData.rallyPointLink}&from=simulator&att_${s.schedule.unitType.canonicalName}=1&target_village_id=${s.schedule.targetVillageData.id}&${makeTroopQueryString(s.units)}]${s.schedule.targetVillageData.name}[/url]`);
                            ...
                            if (useIcons) {
                                ...
                                row.push(translate(INTL.M_TROOP_COMMON_NAMES, { entry: TW.unitTypes.indexOf(s.schedule.unitType) }));
                            }
                ...
                function makeDestinationOutput($target) {
                    ...
                    orderedSnipes.forEach((s) => {
                        ...
                        destVillageSnipes.forEach((svs) => {
                            if (snipeData == null && svs.target == s.schedule.targetVillageData) {
                                snipeData = svs;
                            }
                        });
                    ...
                    destVillageSnipes.forEach((vs) => {
                        ...
                        var headerFormat;
                        if (isCompact) {
                            headerFormat = [translate(INTL.T_SOURCE), translate(INTL.T_LAUNCH_TIME), translate(INTL.T_LANDS_AT), translate(INTL.T_TROOP_REQ)];
                        } else {
                            headerFormat = ['#', translate(INTL.T_SOURCE), translate(INTL.T_LAUNCH_TIME), translate(INTL.T_LANDS_AT), '', translate(INTL.T_TROOPS)];
                        }
    
                        var headerText = `${translate(INTL.T_SNIPES_TO_DST, { target: target.coordText })} (${snipes.length})`;
                        if (isCompact) {
                            lines.push(headerText);
                        } else {
                            lines.push(`[b]${headerText}[/b]`);
                        ...
                        lines.push(makeBbCodeTable(headerFormat, snipes.map((s, i) => {
                            ...
                            row.push(`[url=${s.schedule.sourceVillageData.rallyPointLink}&from=simulator&att_${s.schedule.unitType.canonicalName}=1&target_village_id=${s.schedule.targetVillageData.id}&${makeTroopQueryString(s.units)}]${s.schedule.sourceVillageData.name}[/url]`);
                ...
                function makeAllOutput($target) {
                    ...
                    var headerFormat;
                    if (isCompact) {
                        headerFormat = [translate(INTL.T_SOURCE_VILLA), translate(INTL.T_LAUNCH_TIME), translate(INTL.T_LANDS_AT), translate(INTL.T_TROOP_REQ)];
                    } else {
                        headerFormat = ['#', translate(INTL.T_SOURCE_VILLA), translate(INTL.T_LAUNCH_TIME), translate(INTL.T_LANDS_AT), '', translate(INTL.T_TROOPS)];
                    }
    
                    lines.push(makeBbCodeTable(headerFormat, orderedSnipes.map((s, i) => {
                        var row = [];
                        if (!isCompact) {
                            row.push(i + 1 + '.');
                        }
                        ...
                        row.push(formatDateTime(s.schedule.launchTime), formatDateTime(s.schedule.arrivalTime));
    ...
    */

    {
        ...
        translations.en = (() => {
            ...
            en[INTL.T_MAKE_SNIPES_CHECK_SIGILS] = 'Making snipes and checking for sigils...';
            en[INTL.T_NO_INCS_SELECTED] = 'No incomings were selected for sniping!';
            en[INTL.T_RUN_ON_INCOMINGS] = "This script needs to be ran on the Incomings page, I'll bring you there now.";
            en[INTL.T_SNIPE_TIME_RANGE] = 'Snipes between {{currentTime}} and {{endTime}}';
            ...
            en[INTL.T_INCOMING_SUPPORT_BUFF] = 'Incoming support sent while this is active will travel {{perc}}% faster';
            ...
            en[INTL.T_REFRESH_1] = 'This script will not update when you check/uncheck incomings below!';
            en[INTL.T_REFRESH_2] = 'Run the script again to update with those changes.';
        ...
        translations.br = (() => {
            ...
            br[INTL.T_LOADING_ENHANCER] = 'Carregando potenciador de bloco de notas...';
            br[INTL.T_LOADING_SCRIPT] = 'Carregando EZ-Snipe...';
            br[INTL.T_MAKE_SNIPES_CHECK_SIGILS] = 'Fazendo snips e procurando por sigilos...';
            br[INTL.T_NO_INCS_SELECTED] = 'Nenhum ataque foi selecionado para snip!';
            br[INTL.T_RUN_ON_INCOMINGS] = 'Este script precisa ser executado na pГЎgina Chegando, vou trazer Г  vocГЄ agora';
            ...
            br[INTL.T_TROOP_REQ] = 'Tropa necessГЎria';
            br[INTL.T_TROOPS] = 'Tropas';
            br[INTL.T_USE_TROOP_ICONS] = 'Usar Гcones de tropas';

            br[INTL.T_OPTIONS] = 'OpГ§Гµes';
            br[INTL.T_INCOMING_SUPPORT_BUFF] = 'Apoio recebido enquanto isso estiver ativo vai viajar {{perc}}% mais rГЎpido';
            br[INTL.T_TODAY_AT] = 'Hoje Г s {{time}}';
            br[INTL.T_TOMORROW_AT] = 'AmanhГЈ Г s {{time}}';
            br[INTL.T_ON_AT] = 'Em {{date}} Г s {{time}}';

            br[INTL.T_SNIPE] = 'Snips';
            br[INTL.T_DISPLAY] = 'Mostrar';
            br[INTL.T_NUM_SNIPES_MADE] = '{{count}} snips gerados';
            br[INTL.T_REFRESH_1] = 'Este script nГЈo serГЎ atualizado quando vocГЄ marcar/desmascar as entradas abaixo.';
            br[INTL.T_REFRESH_2] = 'Rode o script novamente para atualizar com aquelas mudanГ§as.';
        ...
        translations.fr = (() => {
        	var fr = {};
        	fr[INTL.T_ALL] = 'Tout';
            fr[INTL.T_ALL_SNIPES] = 'Tous les Snipes';
            fr[INTL.T_BB_CODE] = 'BB-code';
            fr[INTL.T_BY_DST_VILLA] = 'Par village ciblГ©';
            fr[INTL.T_BY_SRC_VILLA] = 'Par village d\'origine';
            fr[INTL.T_COMPACT_MODE] = 'Mode compact';
            fr[INTL.T_LANDS_AT] = 'Heure d\'arrivГ©e';
            fr[INTL.T_LAUNCH_TIME] = 'Heure d\'envoi';
            fr[INTL.T_LOADING_ENHANCER] = 'Chargement du bloc-note optimisГ©...';
            fr[INTL.T_LOADING_SCRIPT] = 'Chargement EZ-Snipe...'
            fr[INTL.T_MAKE_SNIPES_CHECK_SIGILS] = 'Faires le snipes et vГ©rifier les signaux de dГ©tresse...';
            fr[INTL.T_NO_INCS_SELECTED] = 'Aucune attaque sГ©lectionnГ©e pour le snipe!';
            fr[INTL.T_RUN_ON_INCOMINGS] = "Ce script doit ГЄtre exГ©cutГ© sur la page des ordres entrants";
            fr[INTL.T_SNIPE_TIME_RANGE] = 'Snipes entre {{currentTime}} et {{endTime}}';
            fr[INTL.T_SNIPES_FROM_SRC] = 'Snipes depuis {{source}}';
            fr[INTL.T_SNIPES_TO_DST] = 'Snipes vers {{target}}';
            fr[INTL.T_TARGET] = 'Cible';
            fr[INTL.T_TARGET_VILLA] = 'Village ciblГ©';
            fr[INTL.T_SOURCE] = 'Origine';
            fr[INTL.T_SOURCE_VILLA] = 'Village d\'origine';
            fr[INTL.T_TROOP_REQ] = 'Troupes nГ©cessaires';
            fr[INTL.T_TROOPS] = 'Troupes';
            fr[INTL.T_USE_TROOP_ICONS] = 'Utiliser icones troupes';

            //  IN-GAME TEXT - MUST MATCH EXACTLY
            fr[INTL.T_INCOMING_SUPPORT_BUFF] = 'Support entrant voyagera {{perc}}% plus rapidement durant l\'activation';
            fr[INTL.T_TODAY_AT] = 'Aujourd\'hui Г  {{time}}';
            fr[INTL.T_TOMORROW_AT] = 'Demain Г  {{time}}';
            fr[INTL.T_ON_AT] = 'le {{date}} Г  {{time}}';

            fr[INTL.T_OPTIONS] = 'Options d\'affichage';
            fr[INTL.T_OPTIONS_SNIPE] = 'Options de snipe';
            fr[INTL.T_DISPLAY] = 'Affichage: ';
            fr[INTL.T_SNIPE] = 'Snipe';
            fr[INTL.T_MINIMUM_SNIPE_DEF] = 'Minimum de def pour snipe';
            fr[INTL.T_USE_OFFENSIVE_SNIPES] = 'Snipes Off';
            fr[INTL.T_MAX_LAUNCH_TIME] = 'Nombre max d\'heure jusqu\'au lancement';
            fr[INTL.T_NUM_SNIPES_MADE] = '{{count}} snipes faits';
            fr[INTL.T_REFRESH_1] = 'Le script ne sera pas mis Г  jour lorsque vous cochez/dГ©cochez les ordres ci-dessous!';
            fr[INTL.T_REFRESH_2] = 'ExГ©cutez Г  nouveau le script pour mettre Г  jour avec ces modifications.';

            fr[INTL.T_ANTI_INFANTRY] = 'anti-infanterie';
            fr[INTL.T_ANTI_CAVALRY] = 'anti-cavalrie';
            fr[INTL.T_ANTI_ARCHER] = 'anti-archer';

            fr[INTL.M_TROOP_COMMON_NAMES] = [
                'Lancier',
                'P.E',
                'Hache',
                'Archer',
            ...
            fr[INTL.M_TROOP_SHORTHAND_NAMES] = [
                'la',
                'pe',
                'ha',
                'ar',
        ...
        translations.gr = (() => {
            ...
            gr[INTL.T_ALL] = 'ОЊО»О±';
            gr[INTL.T_ALL_SNIPES] = 'ОЊО»О± П„О± snipe';
            gr[INTL.T_BB_CODE] = 'BB Code';
            gr[INTL.T_BY_DST_VILLA] = 'О§П‰ПЃО№ПЊ ПѓП„ПЊП‡ОїП‚';
            gr[INTL.T_BY_SRC_VILLA] = 'О§П‰ПЃО№ПЊ ПЂПЃОїОО»ОµП…ПѓО·П‚';
            gr[INTL.T_COMPACT_MODE] = 'ОЈП…ОјПЂО±ОіО®П‚ ООєОґОїПѓО·';
            gr[INTL.T_LANDS_AT] = 'ОЏПЃО± О¬П†О№ОѕО·П‚';
            gr[INTL.T_LAUNCH_TIME] = 'О¦ПЊПЃП„П‰ПѓО·';
            gr[INTL.T_LOADING_ENHANCER] = 'О¦ПЊПЃП„П‰ПѓО· ОІОµО»П„О№П‰ОјООЅОїП… ПѓО·ОјОµО№П‰ОјО±П„О±ПЃОЇОїП…';
            gr[INTL.T_LOADING_SCRIPT] = 'О¦ПЊПЃП„П‰ПѓО· Script'
            gr[INTL.T_MAKE_SNIPES_CHECK_SIGILS] = 'О”О·ОјО№ОїП…ПЃОіОЇО± snipe ОєО±О№ ОО»ОµОіП‡ОїП‚ ПѓО·ОјО±О№ПЋОЅ';
            gr[INTL.T_NO_INCS_SELECTED] = 'О”ОµОЅ ОП‡ОїП…ОЅ ОµПЂО№О»ОµП‡ОёОµОЇ ОµО№ПѓОµПЃП‡ПЊОјОµОЅОµП‚!';
            gr[INTL.T_RUN_ON_INCOMINGS] = "О¤Ої script П„ПЃОП‡ОµО№ О±ПЂОї П„О· ПѓОµО»ОЇОґО± ОµО№ПѓОµПЃП‡ОїОјООЅП‰ОЅ. О‘ОЅО±ОєО±П„ОµПЌОёП…ОЅПѓО·...";
            gr[INTL.T_SNIPE_TIME_RANGE] = 'Snipes О±ОЅО¬ОјОµПѓО± ПѓП„О№П‚ {{currentTime}} ОєО±О№ ПѓП„О№П‚ {{endTime}}';
            gr[INTL.T_SNIPES_FROM_SRC] = 'Snipes О±ПЂОї {{source}}';
            gr[INTL.T_SNIPES_TO_DST] = 'Snipes ПѓП„Ої {{target}}';
            gr[INTL.T_TARGET] = 'ОЈП„ПЊП‡ОїП‚';
            gr[INTL.T_TARGET_VILLA] = 'О§П‰ПЃО№ПЊ ПѓП„ПЊП‡ОїП‚';
            gr[INTL.T_SOURCE] = 'О ПЃОїОО»ОµП…ПѓО·';
            gr[INTL.T_SOURCE_VILLA] = 'О§П‰ПЃО№ПЊ ПЂПЃОїОО»ОµП…ПѓО·П‚';
            gr[INTL.T_TROOP_REQ] = 'О¤О±П‡. ОјОїОЅО¬ОґО±П‚';
            gr[INTL.T_TROOPS] = 'ОЈП„ПЃО±П„ОµПЌОјО±П„О±';
            gr[INTL.T_USE_TROOP_ICONS] = 'О§ПЃО®ПѓО· ОµО№ОєОїОЅО№ОґОЇП‰ОЅ ПѓП„ПЃО±П„ОµП…ОјО¬П„П‰ОЅ';

            //  IN-GAME TEXT - MUST MATCH EXACTLY
            gr[INTL.T_INCOMING_SUPPORT_BUFF] = 'О— П…ПЂОїПѓП„О®ПЃО№ОѕО· ПѓОµ О±П…П„ПЊ П„Ої П‡П‰ПЃО№ПЊ ОёО± П„О±ОѕО№ОґОП€ОµО№ {{perc}}% ОіПЃО·ОіОїПЃПЊП„ОµПЃО±';
            gr[INTL.T_TODAY_AT] = 'ОЈО®ОјОµПЃО± ПѓП„О№П‚ {{time}}';
            gr[INTL.T_TOMORROW_AT] = 'О‘ПЌПЃО№Ої ПѓП„О№П‚ {{time}}';
            gr[INTL.T_ON_AT] = 'О¤О·ОЅ {{date}} ПѓП„О№П‚ {{time}}';

            gr[INTL.T_OPTIONS] = 'ОЎП…ОёОјОЇПѓОµО№П‚';
            gr[INTL.T_OPTIONS_SNIPE] = 'ОЎП…ОёОјОЇПѓОµО№П‚ snipe';
            gr[INTL.T_DISPLAY] = 'О•ОјП†О¬ОЅО№ПѓО·';
            gr[INTL.T_SNIPE] = 'Snipe';
            gr[INTL.T_MINIMUM_SNIPE_DEF] = 'О•О»О¬П‡О№ПѓП„ОµП‚ О¬ОјП…ОЅОµП‚ Snipe';
            gr[INTL.T_USE_OFFENSIVE_SNIPES] = 'Snipe ОјОµ ОµПЂО№ОёОµП„О№ОєО¬';
            gr[INTL.T_MAX_LAUNCH_TIME] = 'ОњООіО№ПѓП„О· П‰ПЃО± ПЂОїП… ОёО± ОѕОµОєО№ОЅО¬ОµО№ П„Ої Snipe';
            gr[INTL.T_NUM_SNIPES_MADE] = '{{count}} snipe ООіО№ОЅО±ОЅ';
            gr[INTL.T_REFRESH_1] = 'О‘П…П„Ої П„Ої script ОґОµОЅ ОёО± О±ОЅО±ОЅОµП‰ОёОµОЇ О±ОЅ ОµПЂО№О»ООѕОµО№П‚/ОѕОµ-ОµПЂО№О»ООѕОµО№П‚ ОµО№ПѓОµПЃП‡ПЊОјОµОЅОµП‚ ОµПЂО№ОёОПѓОµО№П‚ ОєО±П„П‰!';
            gr[INTL.T_REFRESH_2] = 'О¤ПЃООѕОµ П„Ої script ОѕО±ОЅО± ОіО№О± ОЅО± О±ОЅО±ОЅОµПЋПѓОµО№П‚ ОјОµ П„О№П‚ ОЅОµОµП‚ ОµПЂО№О»ОїОіОП‚.';

            gr[INTL.T_ANTI_INFANTRY] = 'О±ОЅП„О№-ПЂОµО¶О№ОєПЊ';
            gr[INTL.T_ANTI_CAVALRY] = 'О±ОЅП„О№-О№ПЂПЂО№ОєПЊ';
            gr[INTL.T_ANTI_ARCHER] = 'О±ОЅП„О№-П„ОїОѕПЊП„ОµП‚';
            ...
            gr[INTL.M_TROOP_COMMON_NAMES] = [
                ...
                'О‘ОЅО№П‡ОЅОµП…П„О®П‚',
                'О•О»О±П†ПЃПЌ О™ПЂПЂ',
                'О€П†О№ПЂПЂОїП‚ О¤ОїОѕ',
                'О’О±ПЃПЌ О™ПЂПЂ.',
                'ОљПЃО№ПЊП‚',
                'ОљО±П„О±ПЂОО»П„О·П‚',
                'Paladin',
                'О‘ПЃО№ПѓП„ОїОєПЃО¬П„О·П‚',
            ];
        ...
        translations.ru = (() => {
            ...
            ru[INTL.T_ALL] = 'Все';
            ru[INTL.T_ALL_SNIPES] = 'Все снайпы';
            ru[INTL.T_BB_CODE] = 'BB-код';
            ru[INTL.T_BY_DST_VILLA] = 'По целевой деревне';
            ru[INTL.T_BY_SRC_VILLA] = 'По исходной деревне';
            ru[INTL.T_COMPACT_MODE] = 'Компактный режим';
            ru[INTL.T_LANDS_AT] = 'Время прибытия';
            ru[INTL.T_LAUNCH_TIME] = 'Время отправки';
            ru[INTL.T_LOADING_ENHANCER] = 'Загрузка улучшителя блокнота...';
            ru[INTL.T_LOADING_SCRIPT] = 'Загрузка EZ-Snipe...';
            ru[INTL.T_MAKE_SNIPES_CHECK_SIGILS] = 'Создание снайпов и проверка печатей...';
            ru[INTL.T_NO_INCS_SELECTED] = 'Не выбрано входящих атак для снайпинга!';
            ru[INTL.T_RUN_ON_INCOMINGS] = 'Этот скрипт должен быть запущен на странице Входящих, я перенаправлю вас туда сейчас.';
            ru[INTL.T_SNIPE_TIME_RANGE] = 'Снайпы между {{currentTime}} и {{endTime}}';
            ru[INTL.T_SNIPES_FROM_SRC] = 'Снайпы из {{source}}';
            ru[INTL.T_SNIPES_TO_DST] = 'Снайпы в {{target}}';
            ru[INTL.T_TARGET] = 'Цель';
            ru[INTL.T_TARGET_VILLA] = 'Целевая деревня';
            ru[INTL.T_SOURCE] = 'Источник';
            ru[INTL.T_SOURCE_VILLA] = 'Исходная деревня';
            ru[INTL.T_TROOP_REQ] = 'Треб. войск';
            ru[INTL.T_TROOPS] = 'Войска';
            ru[INTL.T_USE_TROOP_ICONS] = 'Использовать иконки войск';

            //  IN-GAME TEXT - MUST MATCH EXACTLY
            ru[INTL.T_INCOMING_SUPPORT_BUFF] = 'Входящая поддержка, отправленная пока это активно, будет путешествовать на {{perc}}% быстрее';
            ru[INTL.T_TODAY_AT] = 'сегодня в {{time}}';
            ru[INTL.T_TOMORROW_AT] = 'завтра в {{time}}';
            ru[INTL.T_ON_AT] = '{{date}} в {{time}}';

            ru[INTL.T_OPTIONS] = 'Параметры отображения';
            ru[INTL.T_OPTIONS_SNIPE] = 'Параметры снайпа';
            ru[INTL.T_DISPLAY] = 'Отображение: ';
            ru[INTL.T_SNIPE] = 'Снайп';
            ru[INTL.T_MINIMUM_SNIPE_DEF] = 'Мин. защита снайпа';
            ru[INTL.T_USE_OFFENSIVE_SNIPES] = 'Атакующие снайпы';
            ru[INTL.T_MAX_LAUNCH_TIME] = 'Макс. часов до запуска';
            ru[INTL.T_NUM_SNIPES_MADE] = 'Создано {{count}} снайпов';
            ru[INTL.T_REFRESH_1] = 'Этот скрипт не обновится, когда вы отметите/снимите отметку с входящих ниже!';
            ru[INTL.T_REFRESH_2] = 'Запустите скрипт снова, чтобы обновить с этими изменениями.';

            ru[INTL.T_ANTI_INFANTRY] = 'против пехоты';
            ru[INTL.T_ANTI_CAVALRY] = 'против кавалерии';
            ru[INTL.T_ANTI_ARCHER] = 'против лучников';

            ru[INTL.M_TROOP_COMMON_NAMES] = [
                'Копейщик',
                'Мечник',
                'Топорщик',
                'Лучник',
            ...
            ru[INTL.M_TROOP_SHORTHAND_NAMES] = [
                'коп',
                'меч',
                'топ',
                'луч',
        ...
        function translate(messageCode, args) {
            ...
            translations.enp = translations.en;
            ...
            if (!dictionary) {
                console.log(`Tried to translate ${messageCode} but server ${serverName} is not supported`);
                debugger;
                return messageCode;
            }

            if (!dictionary.checkedArchers && typeof window.worldHasArchers !== 'undefined' && window.worldHasArchers) {
                objForEach(dictionary, (v, p) => v instanceof Array ? v.splice(3, 1) && v.splice(5, 1) : null);
            ...
            if (isMultiReference || !args || (args && !args.type)) {
                if (dictionary[messageCode]) {
                    ...
                    console.log(`Translation dictionary for '${serverName}' does not have a translation for message code '${messageCode}'`);
                    debugger;
                }
            } else if (!args.type) {
                debugger;
    ...
    //  Build custom translations
    {
        ...
        var customTranslations = (() => {
            ...
            if (typeof translationFormat != 'string') {
                window.console.log('No custom translations detected (translationFormat is undefined or not a string)');
                return;
            }
            ...
            while (lines.length > 0 && (lines[0].trim().length == 0 || lines[0].trim()[0] == '#'))
                lines.splice(0, 1);

            var world = lines.length > 0 ? lines[0].match(/server\s*=\s*(\w+)/) : null;
            if (!world) {
                console.warn('(Custom translation) Missing server name in first line (expected: "server="{name}"; ie "server=en", "server=us", "server=br", etc.) The translation will NOT be loaded.');
                return;
            }
            ...
            for (var i = 1; i < lines.length; i++) {
                ...
                if (!match) {
                    console.warn('(Custom translation) Unknown value at line ' + (i + 1) + ': ', line);
                    continue;
                }
                ...
                if (expectedKeys.indexOf(key) < 0) {
                    console.error(`(Custom translation) Unknown text key '${key}' at line ${i + 1}`)
                    continue;
                }
                ...
                switch (key[0]) {
                    ...
                    case 'M':
                        value = value.split(',');
                        if (value.length != 12) {
                            console.error(`(Custom translation) Expected 12 values for text key '${key}' at line ${i + 1} but got ${value.length}, sample english text:\n${translate(key, 'RAW')}`);
                            continue;
                        }
            ...
            var missingKeys = [];
            expectedKeys.forEach((k) => translation[k] ? null : missingKeys.push(k) && console.warn(`(Custom translation) Missing translation for text code '${k}', sample english text:\n${translate(k, 'RAW')}`));

            window.console.log('(Custom translation) Built custom translation dictionary: ', translation);

            if (missingKeys.length) {
                console.warn(`(Custom translation) ${missingKeys.length} text keys were missing from the custom dictionary. They will use their English versions instead.`);
                missingKeys.forEach((k) => translation[k] = translations.en[k]);
            } else {
                window.console.log('(Custom translation) Successfully generated custom dictionary.');
            }
    ...
    $('#ezs-output').remove();
    var $output = $(`<div id="ezs-output" style="display:inline-block">${translate(INTL.T_LOADING_SCRIPT)}</div>`);
...

    {
...

        {
            ...
            if (href.indexOf('screen=place') >= 0) {
                ...
                `.trim()).insertBefore('#content_value > h3');
                return;
            }
            ...
            if (window.location.href.indexOf('mode=incoming') < 0) {
                ...
                function onClickCustomLink(ev) {
                    ...
                    var landingTime = prompt('Enter the landing time to snipe\n(HH:MM:SS:mmm DD/MM/YYYY)\n(ie 01:23:45:678 22/3/2018)', lastText || '');
                    ...
                    if (timeParts.length != 4) {
                        alert("Time must be of the format: Hours:Minutes:Seconds:Milliseconds");
                        return false;
                    }
                    ...
                    getVillageDataById(targetId, (villageData) => {
                        ...
                        alert(`Saved snipe to ${village.name} at ${landingTime.toUTCString()}`);
                    });

                    return false;
                }

                var shownIdsMessage = 'Village IDs have been displayed for sniping. Use these after running the script on your Incomings page to snipe other players\' villages.';
                ...
                if (href.indexOf('screen=info_village') >= 0) {
                    ...
                    $villageName.empty();
                    $villageName.html(`${villageName}<span class="ezs-decoration">, ID: <a>${id}</a></span>`);
                ...
                if ($villageLinks.length) {
                    ...
                    $villageLinks.each((i, el) => {
                        var id = $(el).attr('href').match(/id=[^\d]*(\d+)/)[1];
                        var $idLabel = $(`<span class="ezs-decoration"><em> (ID: <a>${id}</a>)</em></span>`);
                        $(el).after($idLabel);
            ...
            if ((href.indexOf('screen=overview_villages') < 0 || href.indexOf('mode=incomings') < 0)
                && $('#overview_menu > tbody > tr > td.selected').text().trim() != 'Incoming') {
                $output.remove();
        ...
        if (typeof ezsSettings.minimumSnipeDefense != 'undefined' && typeof ezsSettings.minimumSnipeDefense == 'number') {
            ezsSettings.minimumSnipeDefense = {
                infantry: ezsSettings.minimumSnipeDefense / 2,
                cavalry: ezsSettings.minimumSnipeDefense / 2,
                archer: 0
            }
        } else if (ezsSettings.minimumSnipeDefense && typeof ezsSettings.minimumSnipeDefense == 'object') {
            ezsSettings.minimumSnipeDefense.infantry = ezsSettings.minimumSnipeDefense.infantry || 0;
            ezsSettings.minimumSnipeDefense.cavalry = ezsSettings.minimumSnipeDefense.cavalry || 0;
            ezsSettings.minimumSnipeDefense.archer = ezsSettings.minimumSnipeDefense.archer || 0;
        } else {
            ezsSettings.minimumSnipeDefense = { infantry: 2500, cavalry: 2500, archer: 0 };
        }
        ...
        ezsSettings.maximumTimeUntilLaunch = ezsSettings.maximumTimeUntilLaunch || 24;

        var randomString = () => Math.random().toString().substr(2);
        tcRequire([
            { url: 'https://tylercamp.me/tw/twstats.js?cacheBuster=' + randomString(), indicator: () => window.TW }, // Troop stats - names, off/def, speed, etc.
            {
                url: `https://${window.location.host}/interface.php?func=get_config`, nonJs: true, indicator: () => window.TW && TW.loadedSpeeds, onLoad: (data) => {
                    var $xml = $(data);
                    ...
                    console.log({
                        archersEnabled, paladinEnabled, gameSpeed, unitSpeed
                    });
            ...
            // Get player villages
            var villasUrl = makeSitterUrl(`/game.php?village=${villaId}&screen=overview_villages&mode=units&page=-1`);
            ...
            $.get(villasUrl, (data) => {
                ...
                var $selectedRows = $('#incomings_table tr.nowrap').filter((i, e) => $(e).find('input[type=checkbox]').prop('checked'));
        ...
        function updateIncomings($incomingRows, villas, incomings, serverDate) {
            ...
            $incomingRows.each((i, el) => {
                ...
                if (translate(INTL.T_TOMORROW_AT, { type: 'regex' }).exec(arrivalText) || translate(INTL.T_TODAY_AT, { type: 'regex' }).exec(arrivalText)) {
                    // Format: "tomorrow at HH:MM:SS:mmm";
                    var match;
                    if (
                        !(match = translate(INTL.T_TOMORROW_AT, { type: 'regex' }).exec(arrivalText)) &&
                        !(match = translate(INTL.T_TODAY_AT, { type: 'regex' }).exec(arrivalText))
                    ) {
                        debugger;
                        return;
                    }

                    if (translate(INTL.T_TOMORROW_AT, { type: 'regex' }).exec(arrivalText)) {
                ...
                } else if (translate(INTL.T_ON_AT, { type: 'regex' }).exec(arrivalText)) {
                    // Format: "on DD.MM. at HH:MM:SS:mmm"
                    var match = translate(INTL.T_ON_AT, { type: 'regex' }).exec(arrivalText);
                    if (!match) {
                        debugger;
                        return;
                    }
                    ...
                    console.error(`Couldn't understand arrivalText value '${arrivalText}', likely an incorrect or missing translation for the current server, the script may break`);
                    debugger;
                    return;
                }
        ...
        function onSigilsDoneCreateSnipesAndInterface(incomings, villas) {
            ...
            function makeSnipeOptions() {
                ...
                snipeOptions = (function filterOptions(options) {
                    ...
                    options.forEach((o) => {
                        ...
                        var defensiveStats = calculateTroopStatsByUnitSpeed(o.sourceVillageData.ownTroopsInVilla, o.unitType, ezsSettings.offensiveSnipes ? null : 'Defensive');
                        o.stats = defensiveStats;
                        if (defensiveStats.totalInfDef - ezsSettings.minimumSnipeDefense.infantry < 0 ||
                            defensiveStats.totalCavDef - ezsSettings.minimumSnipeDefense.cavalry < 0 ||
                            defensiveStats.totalArDef - ezsSettings.minimumSnipeDefense.archer < 0) {
                            return;
                        }
                        ...
                        filteredOptions.forEach((fo) => {
                            if (fo.sourceVillageData == o.sourceVillageData && fo.launchTime.valueOf() == o.launchTime.valueOf() && fo.targetVillageData == o.targetVillageData) {
                                exists = true;
                            }
                        });
            ...
            $output.html(`
                ...
                <div style="float:left; line-height:1.5em">
                    ...
                    <div style="padding-left: 1em;">
                        <label for="ezs-select-display">${translate(INTL.T_DISPLAY)} </label>
                        <select id="ezs-select-display">
                            <option value="source">${translate(INTL.T_BY_SRC_VILLA)}</option>
                            <option value="destination">${translate(INTL.T_BY_DST_VILLA)}</option>
                            <option value="all" selected>${translate(INTL.T_ALL)}</option>
                        </select>
                        <br>
                        <input type="checkbox" id="ezs-use-compact" checked>
                        <label for="ezs-use-compact">${translate(INTL.T_COMPACT_MODE)}</label>
                        <br>
                        <input type="checkbox" id="ezs-use-icons" checked>
                        <label for="ezs-use-icons">${translate(INTL.T_USE_TROOP_ICONS)}</label>
                    </div>

                    <h4>${translate(INTL.T_OPTIONS_SNIPE)}</h4>
                    <div style="padding-left: 1em;">
                        <input type="checkbox" id="ezs-offensive-snipes" ${ezsSettings.offensiveSnipes ? 'checked' : ''}>
                        <label for="ezs-offensive-snipes">${translate(INTL.T_USE_OFFENSIVE_SNIPES)}</label>
                        <br>
                        <br>
                        <input type="number" id="ezs-max-launch-time" value="${ezsSettings.maximumTimeUntilLaunch}" style=${numInputStyle}>
                        <label for="ezs-max-launch-time">${translate(INTL.T_MAX_LAUNCH_TIME)}</label>
                        <br>
                        <br>
                        ${translate(INTL.T_MINIMUM_SNIPE_DEF)} (<a target="_blank" href="http://tylercamp.me/tw/#eyJ3b3JsZFNwZWVkIjoiMSIsInN0cnVjdHVyZXMiOnsiQmFycmFja3MiOjcsIlN0YWJsZSI6MywiV29ya3Nob3AiOjEsIkFjYWRlbXkiOjF9LCJ0ZXh0IjoiNTAgc3BlYXJzXG41MCBzd29yZHNcbjI1IGFyY2hlcnMifQ==">reference</a>)
                        <br>
                        - <input type="number" id="ezs-min-inf-def" style=${numInputStyle} value="${ezsSettings.minimumSnipeDefense.infantry}"> ${translate(INTL.T_ANTI_INFANTRY)}
                        <br>
                        - <input type="number" id="ezs-min-cav-def" style=${numInputStyle} value="${ezsSettings.minimumSnipeDefense.cavalry}"> ${translate(INTL.T_ANTI_CAVALRY)}
                        <br>
                        - <input type="number" id="ezs-min-archer-def" style=${numInputStyle} value="${ezsSettings.minimumSnipeDefense.archer}"> ${translate(INTL.T_ANTI_ARCHER)}
                    </div>
                    <br>
                ...
                <div style="float:right; text-align: right;">
                    ...
                    <div style="margin-bottom:1em">
                        ...
                        <input style="width:5.5em;" id="ezs-manual-date" type="text" value="${$('#serverDate').text()}">
                        <br>
                ...
                <p id="ezs-stats">
                    <em>${translate(INTL.T_NUM_SNIPES_MADE, { count: makeSnipeOptions().length })}</em>
                </p>
            ...
            function updateOutput() {
                ...
                $output.find('#ezs-stats em').text(translate(INTL.T_NUM_SNIPES_MADE, { count: snipes.length }));
            ...
            $output.find(`
                ...
                #ezs-select-display,
                input[name^=id_]
                `.trim()).change(updateOutput);

            $output.find('#ezs-recalculate-snipes-btn').click(() => {

                ezsSettings.offensiveSnipes = $('#ezs-offensive-snipes').prop('checked');
            ...
            function makeManualIncomingEntry(newIncoming) {
                var makeTwTimeString = (datetime) => {
                    var time2 = (n) => '0'.repeat(2 - n.toString().length) + n;
                    var time3 = (n) => '0'.repeat(3 - n.toString().length) + n;
                    var timePart = `${time2(datetime.getUTCHours())}:${time2(datetime.getUTCMinutes())}:${time2(datetime.getUTCSeconds())}:${time3(datetime.getUTCMilliseconds())}`;
                    var datePart = `${time2(datetime.getUTCDate())}/${time2(datetime.getUTCMonth() + 1)}/${datetime.getUTCFullYear()}`;
                    return `${timePart} (${datePart})`;
                }

                var $newEntry = $(`
                    <div>
                        To ${newIncoming.destination.name} at ${makeTwTimeString(newIncoming.arrival)}
            ...
            $output.find('#ezs-manual-submit').click(() => {
                ...
                if (!target) {
                    alert('Enter a valid Target Village ID. Open a page to view the target village and look at the "id=" part of the URL, or run this script while viewing a page with a village link.');
                    return;
                }
                if (!launchTimeParts) {
        ...
        function runSnipes($incomingRows, villas) {
            ...
            $output.append(`<p>${translate(INTL.T_MAKE_SNIPES_CHECK_SIGILS)} <span id="ezs-sigil-progress"></span></p>`);
            ...
            targetedVillages = updateIncomings($incomingRows, villas, incomings, serverDate);
            ...
            for (var prop in targetedVillages) {
                ...
                var villaOverviewUrl = makeSitterUrl(`https://${window.location.host}/game.php?village=${villa.id}&screen=overview`);
                pendingUrls.push({
                    villa: villa,
                    ...
                    setTimeout(() => {
                        $.get(url, (data) => {
                            ...
                            $buffs.each((i, el) => {
                                ...
                                if (!$el.attr('title') || $el.attr('title').indexOf('sent') < 0) {
                                    return;
                                }
                                console.log('Found sigil: ', $el);
                                var match = translate(INTL.T_INCOMING_SUPPORT_BUFF, { type: 'parse', content: $el.attr('title') });
                                if (!match) {
                                    debugger;
                                    return;
                                }
                                ...
                                console.log('Updated speed modifier to ' + modifier + '% for ', villa);
                            });
