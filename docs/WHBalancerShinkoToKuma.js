...
if (game_data.locale == "el_GR") {
    langShinko = [
        "Warehouse balancer",
        "О ПЃОїОО»ОµП…ПѓО·",
        "О§П‰ПЃО№ПЊ ПѓП„ПЊП‡ОїП‚",
        "О‘ПЂПЊПѓП„О±ПѓО·",
        "ОћПЌО»Ої",
        "О О·О»ПЊП‚",
        "ОЈОЇОґОµПЃОї",
        "О‘ПЂОїПѓП„ОїО»О® ПЂПЊПЃП‰ОЅ",
        "О”О·ОјО№ОїП…ПЃОіО®ОёО·ОєОµ О±ПЂПЊ П„О·ОЅ Sophie 'Shinko to Kuma'",
        "ОЈПЌОЅОїО»Ої ОѕПЌО»ОїП…",
        "ОЈПЌОЅОїО»Ої ПЂО·О»ОїПЌ",
        "ОЈПЌОЅОїО»Ої ПѓО№ОґО®ПЃОїП…",
        "ОћПЌО»Ої О±ОЅО± П‡П‰ПЃО№ПЊ",
        "О О·О»ПЊП‚ О±ОЅО± П‡П‰ПЃО№ПЊ",
        "ОЈОЇОґОµПЃОї О±ОЅО± П‡П‰ПЃО№ПЊ",
        "Premium exchange",
        "System"
    ];
}
...
if (game_data.locale == "ar_AE") {
    langShinko = [
        ...
        "Ш·Щ…ЩЉ",
        "ШШЇЩЉШЇ",
        "ШҐШ±ШіШ§Щ„ Ш§Щ„Щ…Щ€Ш§Ш±ШЇ",
        "ШЄЩ…ШЄ Ш§Щ„ШЁШ±Щ…Ш¬Щ‡ Щ…Щ† 'Shinko to Kuma'",
        "Щ…Ш¬Щ…Щ€Ш№ Ш§Щ„Ш®ШґШЁ",
        "Щ…Ш¬Щ…Щ€Ш№ Ш§Щ„Ш·Щ…ЩЉ",
        "Щ…Ш¬Щ…Щ€Ш№ Ш§Щ„ШШЇЩЉШЇ",
        "Ш®ШґШЁ Щ„ЩѓЩ„ Щ‚Ш±ЩЉШ©",
        "Ш·Щ…ЩЉ Щ„ЩѓЩ„ Щ‚Ш±ЩЉШ©",
        "ШШЇЩЉШЇ Щ„ЩѓЩ„ Щ‚Ш±ЩЉШ©",
        "Premium exchange",
        "System"
    ];
}
...
if (game_data.locale == "hu_HU") {
    langShinko = [
        ...
        "Agyag",
        "Vas",
        "Nyersanyagok kГјldГ©se",
        "KГ©szГtette: Sophie 'Shinko to Kuma'",
        "Г–sszes fa",
        "Г–sszes agyag",
        "Г–sszes vas",
        "Fa/falu",
...
function displayEverything() {
    ...
    $.get(URLIncRes, function () {
        ...
        .done(function (page) {
            ...
            $.get(URLProd, function () {
                ...
                .done(function (page) {
                    ...
                    if (settings.isMinting == false) {
                        ...
                        if (actualWoodAverage >= 350000 || actualStoneAverage >= 350000 || actualIronAverage >= 350000) {
                            ...
                            alert(`При текущих настройках остаётся слишком много лишних ресурсов, которые невозможно распределить. Попробуйте увеличить параметр «Вместимость фермы» или поднять значение приоритета, чтобы больше деревень получили ресурсы. Обычно это происходит из-за большого количества полностью застроенных деревень или слишком высоких средних запасов при нехватке деревень, которым действительно нужны ресурсы.\nЕсли хотите распределять равномерно, установите «Вместимость фермы» на 99999.\n\nС текущими параметрами вы пытаетесь распределить в среднем ${numberWithCommas(actualWoodAverage)} дерева, ${numberWithCommas(actualStoneAverage)} глины и ${numberWithCommas(actualIronAverage)} железа на каждую деревню, которая не считается маленькой или полностью застроенной (сейчас таких ${numberWithCommas(consideredBuiltOut)}). Это явно плохая идея.`);
                        }
                    ...
                    <tr class='sophRowA'>
                    <td>Фактическое среднее по дереву после корректировки: ${numberWithCommas(actualWoodAverage)}</td>
                    <td>Фактическое среднее по глине после корректировки: ${numberWithCommas(actualStoneAverage)}</td>
                    <td>Фактическое среднее по железу после корректировки: ${numberWithCommas(actualIronAverage)}</td>
                    </tr>
                    </table>`;
                ...
                <div id="sendResources" class="flex-container sophHeader" style="position: relative">
                    ...
                    <div class="content submenu" style="width: 500px;height:500px;z-index:99999">
                        <form id="settings">
                            ...
                            <label for="isMinting">Игнорировать настройки</label></td><td style="padding: 6px;"><input type="checkbox" name="isMinting"></td></tr>
                            <tr>
                            <td style="padding: 6px;">
                            <label for="lowPoints">Приоритет для деревень до</label></td><td style="padding: 6px;"><input type="range" min="0" max="13000" step="10" value="${settings.lowPoints}" class="slider" name="lowPoints" oninput="sliderChange('lowPoints',this.value)">
                            < <output id="lowPoints"></output> очков</td></tr>
                            <tr>
                            <td style="padding: 6px;">
                            <label for="highPoints">Завершённые деревни</label></td><td style="padding: 6px;"><input type="range" min="0" max="13000" step="10" value="${settings.highPoints}" class="slider" name="highPoints" oninput="sliderChange('highPoints',this.value)">
                            > <output id="highPoints"></output> очков</td></tr>
                            <tr>
                            <td style="padding: 6px;">
                            <label for="highFarm">Вместимость фермы</label></td><td style="padding: 6px;"><input type="range" min="0" max="33000" step="10" value="${settings.highFarm}" class="slider" name="highFarm" oninput="sliderChange('highFarm',this.value)">
                            <output id="highFarm"></output> население</td></tr>
                            <tr>
                            <td style="padding: 6px;">
                            <label for="builtOutPercentage">Процент склада для завершённых деревень: </label></td><td style="padding: 6px;"><input type="range" min="0" max="1" step="0.01" value="${settings.builtOutPercentage}" class="slider" name="highFarm" oninput="sliderChange('builtOutPercentage',this.value)">
                            <output id="builtOutPercentage"></output></td></tr>
                            <tr>
                            <td style="padding: 6px;">
                            <label for="needsMorePercentage">Процент склада для приоритетных деревень: </label></td><td style="padding: 6px;"><input type="range" min="0" max="1" step="0.01" value="${settings.needsMorePercentage}" class="slider" name="needsMorePercentage" oninput="sliderChange('needsMorePercentage',this.value)">
                            <output id="needsMorePercentage"></output></td></tr>
                            <tr>
                            <td style="padding: 6px;">
                            <input type="button" class="btn evt-confirm-btn btn-confirm-yes" value="Сохранить" onclick="saveSettings();"/></td></tr>
                            <td colspan="2" style="padding: 6px;">
                            <p style="padding:5px"><font size="1">Скрипт Софии «Shinko to Kuma»</font></p>
                            </td>
                            </table>
                        </form>
                    </div>
                ...
                <tbody id="appendHere">
                    <tr>
                        <td class="sophHeader" colspan=7 width=вЂњ550вЂќ style="text-align:center" >${langShinko[0]}</td>
                    </tr>
                    <tr>
    ...
    function createList() {
        ...
        <tr ${tempRow} height="40">
            ...
            <td> <a href="${targetURL}" class="sophLink" data-toggle="tooltip" title="Дерево на складе: ${targetWood} &#10;Глина на складе: ${targetStone}&#10;Железо на складе: ${targetIron}&#10;Вместимость склада: ${targetCapacity}">${targetName}</a> </td>
        ...
        <button type="button" class="btn btnSophie" name="showStats" style="padding: 10px;width: 300px" onclick="showStats()">Показать избыток/дефицит</button>
        <button type="button" class="btn btnSophie" name="showEndResult" style="padding: 10px;width: 300px" onclick="resAfterBalance()">Показать результат балансировки</button>
        </center></div>`);
        console.log("Finished");
...
function showStats() {
    htmlStats = "<div class='sophRowA' style='width:800px' ><center><h1>Дефицит:</h1><table class='sophHeader'><tr class='sophHeader'><td>Деревня</td><td>Ресурсы</td></tr>";
    ...
    htmlStats += "</table><h1>Избыток:</h1><table class='sophHeader'><tr class='sophHeader'><td>Деревня</td><td>Ресурсы</td></tr>";
...
function resAfterBalance() {
    resBalancedHTML = `<div class='sophRowA' style='width:800px' ><table style='width:100%'><tr class="sophHeader"><td>Деревня</td><td>Очки</td><td>Оставшиеся купцы</td><td colspan="3">Ресурсы</td><td>Вместимость склада</td></tr>`;
