...
function getVillageAttackedTime(cell) {
    ...
    if (attackDay == "Сегодня" || attackDay == "Вчера") {
...
function setDefaultProfile() {
    if (window.top.$('#settingsProfile').val() == "По умолчанию") {
        var newProfile = confirm("Вы не можете установить профиль 'По умолчанию' в качестве профиля по умолчанию. Хотите создать новый профиль?");
        if (newProfile) {
            createProfile();
            setDefaultProfile();
        } else {
        ...
        window.top.$.jStorage.set("profile:По умолчанию", profile);
    }
}
...
function createProfile() {
    var profileName = prompt("Введите название нового профиля:");
...
function deleteProfile() {
    ...
    if (profileName == "По умолчанию") {
        alert("Вы не можете удалить профиль 'По умолчанию'!");
    } else {
        ...
        loadDefaultProfile("По умолчанию");
    }
}
...
function exportProfile() {
    ...
    if (profileName == "По умолчанию") {
        alert("Вы не можете экспортировать профиль 'По умолчанию'!");
    } else {
        var profileSettings = prompt("Настройки профиля " + profileName + ":", "Импортируйте эти настройки, скопировав весь текст: " + profileName + "," + settings);
    }
}

function importProfile() {
    var profileSettings = prompt("Введите настройки профиля для импорта:", "Вставьте сюда настройки профиля");
    ...
    if (cansend && filtersApplied) {
        if (!checkIfNextVillage()) {
            ...
            if (button.hasClass("farm_icon_disabled") || button.html() == undefined) {
                ...
                window.top.UI.ErrorMessage("Эта кнопка недоступна. Пропускаем строку...", 500);
                button.closest('tr').hide();
            }
            else {
                button.click();
...
function editKey(e) {
    if ((e.keyCode <= 37 && e.keyCode >= 40) || (e.keyCode <= 48 && e.keyCode >= 90)) {
        window.top.UI.ErrorMessage("Вы можете вводить только буквы, цифры или стрелки. Пожалуйста, попробуйте другую клавишу.", 1500);
    } else {
        ...
        window.top.UI.SuccessMessage(keyToChar + " теперь назначена на кнопку " + keyToEdit + ".");
...
function setKeyEditMode(n) {
    editingKey = true;
    keyToEdit = n;
    window.top.UI.InfoMessage("Нажмите любую цифру, букву или стрелку, чтобы установить горячую клавишу для кнопки <span style='font-weight:bold;'>" + n + "</span>", 1500);
    return false;
}
...
function loadLanguage(lang) {
    ...
    var profileList = window.top.$.jStorage.get("profileList");
    var defaultProfile = window.top.$.jStorage.get("profile:По умолчанию");
    ...
    window.top.$.getScript(langFile, function () {
        ...
        profileList[0] = "По умолчанию";
        window.top.$.jStorage.set("profileList", profileList);
        window.top.$.jStorage.set("profile:По умолчанию", defaultProfile);
        changeHeader("LA Enhancer");
        showSettings();
    });
}
