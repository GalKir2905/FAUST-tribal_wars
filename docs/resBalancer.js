...
function createMainInterface(){
    ...
    let message_info_factor=`<p>если коэффициент равен 0, деревни будут получать ресурсы только для строительства (из менеджера аккаунта)\n</p>
        <p>если коэффициент равен 1, ресурсы распределяются поровну — после балансировки у всех деревень будет одинаковое количество ресурсов\n</p>
        <p>если, например, указать 0.2, то деревни получат 20% от среднего значения каждого ресурса и дополнительно достаточно ресурсов для строительства (из менеджера аккаунта)</p>`
    let message_info_construction=`<p>укажите, на сколько часов деревни должны быть обеспечены ресурсами для строительства\n</p>
                <p> менеджер аккаунта должен быть активен, а в деревнях должен быть выбран шаблон строительства,
                иначе этот параметр будет проигнорирован `

    let message_info_cluster=`<p>если выбран 1 кластер, ресурсы балансируются глобально; при 2 и более — локально в пределах каждого кластера\n</p>
        <p>чем больше кластеров, тем ниже максимальное время пути и тем локальнее балансировка, что менее эффективно \n</p>
        <p>при каждом запуске кластеры рассчитываются случайно и не всегда оптимально, поэтому следите за максимальной дистанцией \n</p>
        <p>при запуске на карте можно увидеть, как сформированы кластеры и сколько ресурсов отправлено и получено\n</p>`


    let message_max_construction=`<p>если средний коэффициент меньше 0.5 \n</p>
        <p>значение времени строительства автоматически устанавливается на максимально возможное \n</p>
        <p>когда избыток превышает дефицит по всем типам ресурсов\n</p>`

    let html_info=`
    
    <div id="div_container" class="scriptContainer" >
        <div class="scriptHeader">
            <div style=" margin-top:10px;"><h2>Балансировщик ресурсов</h2></div>
        ...
        <div id="div_body" style="height: 600px; overflow-y: auto">
            <center>
                <table id="table_main"  class="scriptTable">
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex;justify-content: center; align-items: center;   ">
                                ...
                                <div><a href="#" onclick="UI.InfoMessage('Сколько купцов вы хотите оставить дома',4000)"><img src="https://dsen.innogamescdn.com/asset/dbeaf8db/graphic/questionmark.png" style="width: 13px; height: 13px"/></a></div>
                            </div>
                        </td>
                    </tr> 
                    <tr>
                        <td>время строительства [часы]</td>
                        <td>
                            <div style="display:flex;justify-content: center; align-items: center;">
                    ...
                    <tr hidden id="tr_merchant_capacity">
                        <td>ёмкость купцов</td>
                        <td>
                            <div style="display:flex;justify-content: center; align-items: center;">
                                <div><input type="number" id="merchant_capacity" class="scriptInput" placeholder="1000" value='1000'></div>
                                <div><a href="#" onclick="UI.InfoMessage('Установите ёмкость купцов на 1000 или 1500',3000)"><img src="https://dsen.innogamescdn.com/asset/dbeaf8db/graphic/questionmark.png" style="width: 13px; height: 13px"/></a></div>
                            </div>
                        </td>
                    </tr> 
                    <tr>
                        <td>максимальное строительство</td>
                        <td>
                            <div style="display:flex;justify-content: center; align-items: center;">
            ...
            <center>
                <input class="btn evt-confirm-btn btn-confirm-yes" type="button" onclick="balancingResources()" style="margin:10px" value="начать">
            </center>
        ...
        <div class="scriptFooter">
            <div style=" margin-top:5px;"><h5>Автор: Costache</h5></div>
        </div>
    </div>`
...
function changeTheme(){
    let html= `
    <h3 style="color:${textColor};padding-left:10px;padding-top:5px">после выбора темы запустите скрипт снова<h3>
    <table class="scriptTable" >
        ...
        <tr>
            <td>
                <select  id="select_theme">
                    ...
                    <option value="theme6">Тема 6</option>
                    <option value="theme7">Тема 7</option>
                    <option value="theme8">Тема 8</option>
                    <option value="theme9">Тема 9</option>
                    <option value="theme10">Тема 10</option>
                </select>
            </td>
            <td>значение</td>
            <td >цвет (hex)</td>
        ...
        <tr >
            <td><input class="btn evt-confirm-btn btn-confirm-yes" type="button" id="btn_save_theme" value="Сохранить"></td>
            <td><input class="btn evt-confirm-btn btn-confirm-yes" type="button" id="btn_reset_theme" value="Темы по умолчанию"></td>
            <td></td>
        </tr>
    ...
    $("#btn_save_theme").on("click",()=>{
        ...
        for(let i=0;i<colours.length-1;i++){
            if(colours[i].match(/#[0-9 A-F]{6}/) == null ){
                UI.ErrorMessage("неверный цвет: "+colours[i])  
        ...
        localStorage.setItem(localStorageThemeName, JSON.stringify(Array.from(mapTheme.entries())))
        console.log("сохранены цвета для: "+selectedTheme)
        UI.SuccessMessage(`цвета сохранены для: ${selectedTheme} \n запустите скрипт снова`,1000)
    ...
    $("#btn_reset_theme").on("click",()=>{
        ...
        UI.SuccessMessage("запустите скрипт снова",1000)
...
async function balancingResources(){
    ...
    console.log("список производств",list_production)
    console.log("использование ферм",map_farm_usage)
    console.log("входящие ресурсы",map_incoming)
    console.log("ресурсы для МА",map_resources_get_AM)
    ...
    console.log("координаты для k-means")
    ...
    console.log("производство по кластерам",list_production_cluster)
    ...
    console.log("статистика кластеров",list_clusters_stats)
    ...
    console.log("карта количества купцов",map_nr_merchants)
    for(let i=0;i<list_production.length;i++){
        let nr_merchants=0
    ...
    console.log("производство после баланса",list_production)
    console.log("отправки ресурсов",list_launches)
    console.log("массовые отправки",list_launches_mass)
    console.log("данные для карты",map_draw_on_map)

    let stop= new Date().getTime()
    console.log("время обработки: "+(stop-start))
...
function hitCountApi(){
    $.getJSON(`https://api.counterapi.dev/v1/${countNameSpace}/${countApiKey}/up`, response=>{
        console.log(`Скрипт запущен: ${response.count} раз(а)`);
    });
    if(game_data.device !="desktop"){
        $.getJSON(`https://api.counterapi.dev/v1/${countNameSpace}/${countApiKey}_phone/up`, response=>{
            console.log(`Скрипт запущен на мобильном: ${response.count} раз(а)`);
        });
    }
    ...
    try {
        $.getJSON(`https://api.counterapi.dev/v1/${countNameSpace}/${countApiKey}_scriptUsers`, response=>{
            console.log(`Всего пользователей: ${response.count}`);
        }); 
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        window.setTimeout(function(){
                            ...
                            UI.SuccessMessage("получение страницы производства: "+urls.length)
                        },200-diff)
                    },
                    error: (err)=>{
                        reject(err)
            ...
            if (urls.length >= 0 && current_url!="stop") {
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        window.setTimeout(function(){
                            ajaxRequest (list_pages)
                            UI.SuccessMessage("получение страницы входящих торговцев: "+urls.length)
                        },200-diff)
                    },
                    error:(err)=>{
                        reject(err)
...
async function createTable(list_launches,obj_stats,list_production,list_clusters_stats){
    ...
    let html_prod_table=`
        ...
        <tr>
            <td style="width:3%">№</td>
            <td style="width:35%">цель</td>
            <td><a href="#" id="sort_distance"><font color="${textColor}">макс. дистанция</font></a></td>
            <td><a href="#" id="sort_total"><font color="${textColor}">всего отправлено</font></a></td>
    ...
    for(let i=0;i<list_launches.length;i++){
        ...
        html_prod_table+=`
            <tr id="delete_row" >
                ...
                <td><input class="btn evt-confirm-btn btn-confirm-yes btn_send" target_id="${target_id}" data='${data}'  type="button" value="отправить"></td>
    ...
    let html_stats_table=`
        ...
        <tr>
            <td><input class="btn evt-confirm-btn btn-confirm-yes" id="btn_result" type="button" value="результаты"></td>
            <td><input class="btn evt-confirm-btn btn-confirm-yes" id="btn_cluster" type="button" value="кластеры"></td>
...
function createTableResults(list_production){
    ...
    <tr>
        <td>координаты</td>
        <td ><a href="#" id="order_points"><font  color="${textColor}">очки</font></a></td>
        <td style="width:10%"><a href="#" id="order_merchants"><font  color="${textColor}">купцы</font></a></td>
        <td >
            <img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/main.png"/>
            <a href="#" id="order_hours"><font  color="${textColor}">[часы]</font></a>
        </td>
        <td colspan="2">
    ...
    $("#order_points").on("click",()=>{
        ...
        console.log("сортировка по очкам")
    ...
    $("#order_merchants").on("click",()=>{
        ...
        console.log("сортировка по купцам")
        $(".popup_box_close").click()
        createTableResults(list_production)
    ...
    $("#order_hours").on("click",()=>{
        ...
        console.log("сортировка по времени строительства")
    ...
    $(".order_deficit").on("click",()=>{
        ...
        console.log("сортировка по дефициту/избытку")
        $(".popup_box_close").click()
        createTableResults(list_production)
    ...
    $("#order_wh").on("click",()=>{
        ...
        console.log("сортировка по вместимости склада")
        $(".popup_box_close").click()
        createTableResults(list_production)
...
function createTableClusters(list_clusters_stats){
    ...
    <tr>
        <td style="width:5%">№</td>
        <td >деревень в кластере</td>
        <td >центр кластера</td>
        <td style="width:50%">ресурсы</td>
        <td >макс. дистанция</td>   
    </tr>`
            ...
            if (urls.length >= 0 && current_url!="stop") {
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        window.setTimeout(function(){
                            ajaxRequest (list_pages)
                            UI.SuccessMessage("получение страницы строительства МА: "+urls.length)
                        },200-diff)
                    },
                    error:(err)=>{
                        reject(err)
            ...
            if (urls.length >= 0 && current_url!="stop") {
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        window.setTimeout(function(){
                            ajaxRequest (list_pages)
                            UI.SuccessMessage("получение страницы зданий: "+urls.length)
                        },200-diff)
                    },
                    error: (err)=>{
                        reject(err)
    ...
    try {
        ...
        if(document.getElementById(`info_extra${obj.villageId}`) == null ){
            ...
            let html_info=`
                ...
                <center><font color="${textColor}"  class="shadow20" style="position:absolute;left:${leftImg};top:${topImg};width:14px;height:14px;z-index:4;margin-left:0px;; font-size: 12px">№:${obj.label_cluster} </font></center>
                <center><font color="${greenColor}"  class="shadow20" style="position:absolute;left:${leftImg};top:${topImg};width:14px;height:14px;z-index:4;margin-left:0px;margin-top:11px; font-size: 12px">${parseInt(obj.total_resources_get/1000)} тыс. </font></center>
                <center><font color="${redColor}"  class="shadow20" style="position:absolute;left:${leftImg};top:${topImg};width:14px;height:14px;z-index:4;margin-left:0px;margin-top:23px; font-size: 12px">${parseInt(obj.total_resources_send/1000)} тыс. </font></center>
                `
