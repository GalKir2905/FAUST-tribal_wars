...
var langShinko = [
    "Отправка ресурсов для чеканки с усилением флага",
    "Введите координату получателя",
    "Сохранить",
    "Автор",
    "Игрок",
    "Деревня",
    "Очки",
    "Координата получателя",
    "Оставить % склада",
    "Пересчитать отправку",
    "Отправка ресурсов",
    "Деревня отправителя",
    "Деревня получателя",
    "Расстояние",
...
if (game_data.locale == "el_GR") {
    langShinko = [
        ...
        "О•О№ПѓО¬ОіОµП„Оµ П„О№П‚ ПѓП…ОЅП„ОµП„О±ОіОјООЅОµП‚ - ПѓП„ПЊП‡Ої",
        "О‘ПЂОїОёО®ОєОµП…ПѓО·",
        "О”О·ОјО№ОїП…ПЃОіПЊП‚",
        "О О±ОЇОєП„О·П‚",
        "О§П‰ПЃО№ПЊ",
        "О ПЊОЅП„ОїО№",
        "ОЈП„ПЊП‡ОїП‚",
        "О”О№О±П„О®ПЃО·ПѓОµ П„Ої % О‘ПЂОїОёО®ОєО·П‚ ОєО¬П„П‰ О±ПЂПЊ",
        "ОҐПЂОїО»ОїОіО№ПѓОјПЊП‚ ПЂПЊПЃП‰ОЅ/О±О»О»О±ОіО® ПѓП„ПЊП‡ОїП…",
        "О‘ПЂОїПѓП„ОїО»О® ПЂПЊПЃП‰ОЅ",
        "О ПЃОїОО»ОµП…ПѓО·",
    ...
    htmlCode = `
            <div id="sendResources" border=0>
                <table id="tableSend" width="100%">
                    <tbody id="appendHere">
                        <tr>
                            <td class="sophHeader" colspan=7 width=вЂњ550вЂќ style="text-align:center" >${langShinko[10]}</td>
                        </tr>
                        <tr>
...
function compareDates(x) {
    ...
    console.log("проверено " + hours + " часов назад для списка деревень");
    return hours;
}
